...
<div class="flex min-h-[100dvh] items-center justify-center p-4 auth-bg">
	<div class="w-full max-w-[420px]">
		<div class="text-center mb-8">
			...
			<h1 class="text-2xl font-bold text-foreground tracking-tight dark:text-white">Forgot password</h1>
			...
			<Card.Content class="p-6 sm:p-8">
				{#if success}
					<div class="space-y-6">
						<div class="text-center py-3">
							<div class="mx-auto w-12 h-12 rounded-full bg-emerald-50 dark:bg-emerald-500/10 flex items-center justify-center mb-4">
							...
							<p class="text-sm text-muted-foreground mt-1">
								If an account exists for <span class="text-foreground font-medium">{email}</span>, we've sent a reset link.
							</p>
						</div>
						<div class="flex flex-col gap-2.5">
							<Button onclick={handleResend} variant="outline" class="w-full h-11 border-border bg-card text-foreground hover:bg-accent rounded-xl" disabled={loading}>
								{#if loading}
							...
							<Button variant="ghost" onclick={goToLogin} class="w-full h-11 text-muted-foreground hover:text-foreground rounded-xl">
								Back to Login
							</Button>
						</div>
					</div>
				{:else}
					<form onsubmit={handleSubmit} class="space-y-5">
						...
						<Button type="submit" class="w-full h-11 text-sm font-semibold bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl transition-all shadow-card" disabled={!email || loading}>
							{#if loading}
								<LoaderIcon class="h-4 w-4 mr-2 animate-spin" />
								Processing...
							{:else}
